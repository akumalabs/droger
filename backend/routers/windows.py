"""Windows install command generator. Public listing + authenticated command builder."""
...
async def windows_script(payload: WindowsScriptReq, user: dict = Depends(current_user)):
    cmd = build_windows_command(payload.version, payload.password, payload.rdp_port)
